...
def run_sim(opt, device):
    ...
    # Running Federated Learning for x times more data
    times_more = [50, 20, 10, 5, 2, 1]
    labels = ["FL_w_%d_times_more" % times for times in times_more]
    ...
    for sim_i in range(
        ...
        for trial_i in range(
            ...
            # Create a list of FL simulations for different cache sizes
            Sim_List = []
            for times in times_more:
                params["n_cache"] = int(opt.n_cache * times)
                Sim_List.append(
                    Sim_FL(
            ...
            # Run simulations for cache sizes
            for i, Sim_Model in enumerate(Sim_List):
                pbar.set_description(
                    "Running FL for %d times more data" % times_more[i]
                )
                Sim_Model.sim_round(
                    0,
            ...
            # Save results for each cache size
            for i, Sim_Model in enumerate(Sim_List):
                Sim_Model.save_infos(trial_loc, times_more[i])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n-cache", type=int, default=2, help="Number of cached samples per device"
    )
    parser.add_argument(
        "--run-loc",
        type=str,
    ...
    parser.add_argument(
        "--unc-type", type=str, default="Random", help="Uncertainty type to use"
    )
    parser.add_argument(
        "--dataset-type",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Number of epochs to train for Federated Learning Agents",
    )
