...
def run_sim(opt,device):
    ...
    for sim_i in range(opt.init_sim, opt.init_sim+opt.n_sim):
        ...
        for trial_i in range(opt.init_trial,opt.init_trial+opt.n_trial):
            ...
            Random_Model = Sim(params,"Random",device,copy.deepcopy(Unc_Model.model))
            params["unc_type"] = "Entropy"
            Entropy_Model = Sim(params,"Entropy",device,copy.deepcopy(Unc_Model.model))
            params["unc_type"] = "LeastConfidence"
            LeastConfidence_Model = Sim(params,"LeastConfidence",device,copy.deepcopy(Unc_Model.model))
            params["unc_type"] = "MarginSampling"
            MarginSampling_Model = Sim(params,"MarginSampling",device,copy.deepcopy(Unc_Model.model))


            x_dist, N_x = Random_Model.create_xdist(trial_i*simcoef_int+simsum_int,obs_classes,y_train)
                
            obs_inds = Random_Model.create_obs_ind(N_x,y_train,trial_i*simcoef_int+simsum_int)

            pbar.set_description("Running Random")
            Random_Model.sim_round(0,trial_i*simcoef_int+simsum_int,X_train,y_train,test_data,base_inds,obs_inds,train_embs)
            pbar.set_description("Running Entropy")
            Entropy_Model.sim_round(0,trial_i*simcoef_int+simsum_int,X_train,y_train,test_data,base_inds,obs_inds,train_embs)
            pbar.set_description("Running LeastConfidence")
            LeastConfidence_Model.sim_round(0,trial_i*simcoef_int+simsum_int,X_train,y_train,test_data,base_inds,obs_inds,train_embs)
            pbar.set_description("Running MarginSampling")
            MarginSampling_Model.sim_round(0,trial_i*simcoef_int+simsum_int,X_train,y_train,test_data,base_inds,obs_inds,train_embs)

            pbar.set_description("Saving results")
            Random_Model.save_infos(trial_loc,"Random")
            Entropy_Model.save_infos(trial_loc,"Entropy")
            LeastConfidence_Model.save_infos(trial_loc,"LeastConfidence")
            MarginSampling_Model.save_infos(trial_loc,"MarginSampling")

            plot_accs([Random_Model.accs,Entropy_Model.accs,LeastConfidence_Model.accs,MarginSampling_Model.accs],["Random","Entropy","LeastConfidence","MarginSampling"],trial_loc+"/Accs.jpg")
        ...
        combine_sims(run_ids,run_i_loc,run_i_loc,["Random","Entropy","LeastConfidence","MarginSampling"],name="trial")
