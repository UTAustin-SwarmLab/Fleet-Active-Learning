...
def run_sim(opt, device):
    ...
    for sim_i in range(
        ...
        for trial_i in range(
            ...
            for i, Sim_Model in enumerate(Sim_List):
                Sim_Model.save_infos(trial_loc, str(times_more[i]) + "_times_more")
    ...
    # List of sim_types
    sim_types = [str(times) + "_times_more" for times in times_more]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config-path", type=str, default="./configs/adverseweather_fl_test.json"
    )
