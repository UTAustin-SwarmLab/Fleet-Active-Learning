...
class Sim:
    def __init__(self, params, sim_type, device, model):
        ...
        :param sim_type: Type of simulation - Distributed, Centralized, Interactive
        ...
        self.accs = np.zeros((1, self.n_rounds + 1))
        self.accs_matrix = np.zeros((1, self.n_rounds + 1, self.n_class, self.n_class))
        self.min_accs = np.zeros((1, self.n_rounds + 1))
    ...
    def create_base_inds(self, y, sim_i, seed):
        self.base_seeds.append(seed)
        np.random.seed(seed)

        inds = list()
        if self.dirichlet_base:
            x_dist = np.random.dirichlet(
                np.repeat(self.dirichlet_base_alpha, self.n_class)
            )
        else:
            x_dist = np.random.rand(self.n_class)
        x_dist = x_dist / sum(x_dist)

        N_x = [0 for i in range(self.n_class)]
        ...
        fulls = []
        while n_lim != 0:
            n_cache_cand = saferound(
                x_dist[~np.isin(np.arange(len(x_dist)), fulls)]
                * n_lim
                / sum(x_dist[~np.isin(np.arange(len(x_dist)), fulls)]),
                places=0,
            )
        ...
        for i in range(self.n_class):
            ind_s = random.sample(np.argwhere(y == i).reshape(-1).tolist(), k=N_x[i])
            inds.extend(ind_s)
    ...
    def create_xdist(self, sim_i, y):
        np.random.seed(sim_i)

        x_dist = np.zeros((self.n_device, self.n_class))
        if self.dirichlet:
            for i in range(self.n_device):
                x_dist[i, :] = np.random.dirichlet(
                    np.repeat(self.dirichlet_alpha, self.n_class)
                )
        else:
            for i in range(self.n_device):
        ...
        for i in range(self.n_device):
            ...
            while n_lim != 0:
                n_cache_cand = saferound(
                    x_dist[i, ~np.isin(np.arange(len(x_dist[i])), fulls)]
                    * n_lim
                    / sum(x_dist[i, ~np.isin(np.arange(len(x_dist[i])), fulls)]),
                    places=0,
                )
                f = [arg for arg in args if (arg not in fulls)]
                for g, arg in enumerate(f):
    ...
    def create_dataset(self, X, y):
        if self.dataset_type == "MNIST":
            transform = trfm.Compose(
                [trfm.ToTensor(), trfm.Normalize((0.1307), (0.3081))]
            )
    ...
    def obtain_logits(self, X_train, y_train, inds):
        ...
        test_loader = torch.utils.data.DataLoader(
            dataset, batch_size=self.test_b_size, shuffle=False
        )
    ...
    def coreset_obtain_embeddings(self, X_train, y_train, all_inds):
        ...
        with torch.no_grad():
            ...
            loader = torch.utils.data.DataLoader(
                dataset_i, batch_size=self.test_b_size, shuffle=False
            )
            ind = 0
            for x, _ in loader:
                x = x.to(self.device)
        ...
        with torch.no_grad():
            ...
            for x, _ in loader:
                ...
                for c in range(self.n_class):
                    embeddings[
                        ind : ind + len(x), emb_size * c : emb_size * (c + 1)
                    ] = emb * (-1 * batchProbs[:, c].reshape(-1, 1))
                    embeddings[ind : ind + len(x), emb_size * c : emb_size * (c + 1)][
                        y_preds == c
                    ] = emb[y_preds == c] * (
                        1 - batchProbs[:, c][y_preds == c].reshape(-1, 1)
                    )
    ...
    def cache_inds(self, all_inds, obs_inds, unc_scores):
        ...
        for i in range(self.n_device):
            ...
            cached_inds.extend(
                np.array(inds)[np.argsort(scores_i)[-self.n_cache :]].tolist()
            )
    ...
    def reset_model(self):
        if self.dataset_type == "MNIST" or self.dataset_type == "CIFAR10":
            ...
            if self.params["use_embeddings"]:
                ...
                self.model.fc = nn.Linear(self.model.emb_size, self.n_class).to(
                    self.device
                )
    ...
    def sim_round(
        self, sim_i, sim_seed, X_train, y_train, testset, base_inds, obs_ind, train_embs
    ):
        self.sim_i = sim_i
        ...
        for round_i in range(self.n_rounds):
            if self.sim_type in [
                ...
                if self.unc_type == "coreset":
                    embeddings = self.coreset_obtain_embeddings(
                        X_train, y_train, all_indices
                    )
                elif self.unc_type == "badge":
                    embeddings = self.badge_obtain_embeddings(
                        X_train, y_train, all_indices
                    )
                elif self.unc_type == "clip":
                    embeddings = self.clip_obtain_embeddings(
                        X_train, y_train, all_indices, train_embs
                    )

                distances = pairwise_distances(embeddings, metric="euclidean")
                M = 1 / (1 + 0.01 * distances)

                M, M_max = self.create_M_max_M(
            ...
            self.dataset_ind[sim_seed].append(
                self.dataset_ind[sim_seed][-1] + cached_inds
            )

            self.reset_model()
            trainset = self.create_traindataset(
                X_train[tuple([list(set(self.dataset_ind[self.sim_seed][-1]))])],
                y_train[tuple([list(set(self.dataset_ind[self.sim_seed][-1]))])],
            )
            train_model(
                self.model,
                trainset,
...
class Sim_Detect(Sim):
    def __init__(self, params, sim_type, device, model, init_results):
        ...
        self.precision = np.zeros((1, 2))
        self.recall = np.zeros((1, 2))
        self.map50 = np.zeros((1, 2))
        self.map50_95 = np.zeros((1, 2))
        self.fitness = np.zeros((1, 2))
    ...
    def sim_round(
        ...
        trainset = create_detection_dataset(
            X_train[tuple([list(set(self.dataset_ind[self.sim_seed][-1]))])], testset
        )
        self.model.train(
            data=trainset,
            ...
            val=False,
            pretrained=True,
            batch=self.params["b_size"],
            verbose=False,
            plots=False,
            cache=self.params["cache_all"],
            workers=self.params["n_workers"],
        )
...
class Sim_FL(Sim):
    ...
    def FL_train(self, local_inds, X_train, y_train):
        ...
        for local_ind in local_inds:
            trainset = self.create_traindataset(
                X_train[tuple([local_ind])], y_train[tuple([local_ind])]
            )
    ...
    def sim_round(
        ...
        for round_i in range(self.n_rounds):
            if self.params["FL_type"] == "All":
                ...
                for i in range(self.n_device):
                    local_inds = [
                        cached_inds[i * self.n_cache : (i + 1) * self.n_cache]
                        for i in range(self.n_device)
                    ]
