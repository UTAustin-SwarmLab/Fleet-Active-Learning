...
"""
class MNIST_Sim:
    def __init__(self,params,device,model):
...
class MNIST_Sim_CoreSets(MNIST_Sim):

    def __init__(self,params,sim_type,device,model):
        super().__init__(params,device,model)
        self.sim_type = sim_type

    def obtain_embeddings(self,dataset,obs_inds,base_inds):
        ...
        with torch.no_grad():
            ...
            for i in range(self.n_device):
                ...
                dataset_i = self.create_dataset(dataset.data[obs_inds[i]],dataset.targets[obs_inds[i]])
            ...
            dataset_i = self.create_dataset(dataset.data[base_inds],dataset.targets[base_inds])
            loader = torch.utils.data.DataLoader(dataset_i, batch_size=self.test_b_size, shuffle=False)
            ind = 0
            for x,_ in loader:
                x = x.to(self.device)
    ...
    def sim_round(self,sim_i,sim_seed,train_data,testset,base_inds,obs_ind):
        ...
        for round_i in range(self.n_rounds):            
            ...
            embeddings,base_embeddings = self.obtain_embeddings(train_data,obs_ind[round_i],self.dataset_ind[sim_seed][-1])
            ...
            self.model.apply(init_weights)
            trainset = self.create_traindataset(train_data.data[list(set(self.dataset_ind[self.sim_seed][-1]))],train_data.targets[list(set(self.dataset_ind[self.sim_seed][-1]))])
            train_model(self.model,trainset)

            self.accs[sim_i,round_i+1] = test_model(self.model,testset)

class MNIST_Sim_Gradient(MNIST_Sim_CoreSets):
        ...
        with torch.no_grad():
            ...
            for i in range(self.n_device):
                ...
                for x,_ in loader:
                    ...
                    # Matrix version of the above code faster
...
class CIFAR10_Sim_CoreSets(CIFAR10_Sim):

    def __init__(self,params,sim_type,device,model):
        super().__init__(params,device,model)
        self.sim_type = sim_type
    
    obtain_embeddings = MNIST_Sim_CoreSets.obtain_embeddings

    sim_round = MNIST_Sim_CoreSets.sim_round

class CIFAR10_Sim_Gradient(CIFAR10_Sim_CoreSets):

    def __init__(self,params,sim_type,device,model):
        super().__init__(params,sim_type,device,model)
    
    obtain_embeddings = MNIST_Sim_Gradient.obtain_embeddings
"""
